...
export class TypeModalComponent extends MaterializeModal {
    ...
    selectType(event: Event, property: any, openModal = true):  void {
        if (event) {
            event.preventDefault();
        }
        let entity: string;
        ...
        if (this.definition) {
            this.properties = Object.keys(this.definition.schema.properties)
                .map((key) => {return {value: this.definition.schema.properties[key], key: key}; });
        }
